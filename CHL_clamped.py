...
directory = '/home/yoshi82/projects/def-luczak/yoshi82/01_bioplausible/with_delay13_inp12_clamped_f120_c120_ada9'
# list for AdaGrad
dy_squared  = []
...
## show specific hidden activation
## 
##
def show_specifc_activation(hidden_free_activations, hidden_clamped_actvations, fixed_node, n_activaitons, time):
  fig, axs = plt.subplots(figsize=(8,6))

  hidden_free_activations   = np.squeeze(hidden_free_activations).T
  hidden_clamped_actvations = np.squeeze(hidden_clamped_actvations).T
  
  plt.plot(hidden_free_activations[fixed_node, 0:99], label='free phase')
  plt.plot(hidden_clamped_actvations[fixed_node, 0:99], label='clamped phase')
  plt.xlabel('Time step( ' + str(time) + ' x ' + str(n_activaitons) + ' examples)',fontsize=15)
  axs.legend(bbox_to_anchor=(1.1, 1.05), fontsize=13)
...
## 
##
def show_activation_map(ax, hidden_activations, output_activations, n_activaitons, time, subtitle):
  hidden_activations = np.squeeze(hidden_activations).T
  output_activations = np.squeeze(output_activations).T
  ax.set_title(subtitle)
  activations = np.concatenate((hidden_activations, output_activations))
  sns.heatmap(activations, cmap='jet', ax=ax)
  ax.set_xlabel('Time step( ' + str(time) + ' x ' + str(n_activaitons) + ' examples)',fontsize=15)
  ax.set_ylabel('Activations (nodes)',fontsize=15)

## show heatmaps for both free and clamped phase activations 
## 
##
def show_both_phase_activations(hidden_free_activations, output_free_activations, hidden_clamped_activations, output_clamped_activations, n_activaitons, time):
  fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16,6))
  show_activation_map(ax1, hidden_free_activations, output_free_activations, n_activaitons, time, 'Free phase activations')
  show_activation_map(ax2, hidden_clamped_activations, output_clamped_activations, n_activaitons, time, 'Clamped phase activations')
  plt.show()
  plt.clf()
...
#      print('accuracy for testing: '  + str(test_acc.cpu().numpy()))
      ...
      # ## showing the activation maps
      # print('Train phase activations (' + str(n_activations) + ' examples)')
      # show_both_phase_activations(train_free_hidden_activations, train_free_output_activations, train_clamped_hidden_activations, train_clamped_output_activations, n_activations, time )

      # print('Test phase activations (' + str(n_activations) + ' examples)')
      # show_both_phase_activations(test_free_hidden_activations, test_free_output_activations, test_clamped_hidden_activations, test_clamped_output_activations, n_activations, time )

      # print(' A specific hidden node activation on the free and clapmed phases')
      # show_specifc_activation(train_free_hidden_activations, train_clamped_hidden_activations, 5, n_activations, time)
