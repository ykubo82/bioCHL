...
## return clamped input or output
...
def eqprop_update(negative_acts, positive_acts, ws, bs, learning_rate, beta, bidirectional, l2_loss = None, gamma=0.5, dy_squared=None):
    ...
    w_grads = [-(pa_pre.float().cuda().t() @ pa_post.float().cuda() - na_pre.float().cuda().t() @ na_post.float().cuda())/float(n_samples) for na_pre, na_post, pa_pre, pa_post in izip_equal(negative_acts[:-1], negative_acts[1:], positive_acts[:-1], positive_acts[1:])]
...
def run_eqprop_training_update(x_data, y_data, layer_states: Sequence[IDynamicLayer], beta: float, random_flip_beta: bool,
    ...
    ## if prediction is true, we have to use a different function nto get all activations for the negative phase
    if pred:
    ...
    ## if prediction is true, call the prediction function
    if pred:
      ...
      ## prepare for using only every 2 points
